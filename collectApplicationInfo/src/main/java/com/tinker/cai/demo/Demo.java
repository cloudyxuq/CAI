    ...
    public void sendMessage(Boolean syn, final Boolean bro, PDU pdu, String addr)
        ...
        if (!syn) {
            ...
            for(int i =0;i<response.getResponse().getVariableBindings().size();i++){
            	System.out.println(response.getResponse().get(i));
            	
            }
            /**
             * 输出结果：
             * Synchronize(同步) message(消息) from(来自) 192.168.4.12/161
request(发送的请求):GET[requestID=1875732809, errorStatus=Success(0), errorIndex=0, VBS[1.3.6.1.2.1.1.5.0 = Null]]
response(返回的响应):RESPONSE[requestID=1875732809, errorStatus=Success(0), errorIndex=0, VBS[1.3.6.1.2.1.1.5.0 = Lenovo-PC]]
    ...
    public static void main(String[] args) {
        ...
        Demo manager = new Demo(ver2c);
        // 构造报文
        PDU pdu = new PDU();
         //PDU pdu = new ScopedPDU();
        // Windows8
        pdu.add(new VariableBinding(new OID("1.3.6.1.2.1.1.5.0")));//OID_RemoteName
        pdu.add(new VariableBinding(new OID("1.3.6.1.2.1.1.1.0")));//OID_sysDescr   
        pdu.add(new VariableBinding(new OID("1.3.6.1.2.1.1.2.0")));//OID_sysObjectID   
        pdu.add(new VariableBinding(new OID("1.3.6.1.2.1.1.3.0")));//OID_sysUpTime  
        pdu.add(new VariableBinding(new OID("1.3.6.1.2.1.25.2.2.0")));//总内存 支持windows2003及以上（单位KB）
        ...
        try {
            ...
            manager.sendMessage(false, true, pdu, "udp:192.168.4.254/161");//192.168.1.233 WinServer2008服务器
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
